...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGTemplate)
//Using Name Space to avoid variable name conflict with engine code. Just add it
#define LOCTEXT_NAMESPACE "PCGTemplate"
...
{
	return MakeShared<FPCGTemplateElement>();
}

bool FPCGTemplateElement::ExecuteInternal(FPCGContext* Context) const
{
	//Store PCG Settings. Use it for get access to all variables we declared or adjusted in PCG Settings in header
	const UPCGTemplateSettings* Settings = Context->GetInputSettings<UPCGTemplateSettings>();
	check(Settings);
