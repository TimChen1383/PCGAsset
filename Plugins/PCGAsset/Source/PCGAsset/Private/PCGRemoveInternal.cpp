...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGRemoveInternal)
//Using Name Space to avoid variable name conflict with engine code. Just add it
#define LOCTEXT_NAMESPACE "PCGRemoveInternal"
...
FPCGElementPtr UPCGRemoveInternalSettings::CreateElement() const
{
	return MakeShared<FPCGRemoveInternalElement>();
}

bool FPCGRemoveInternalElement::ExecuteInternal(FPCGContext* Context) const
{
	//Store PCG Settings. Use it for get access to all variables we declared or adjusted in PCG Settings in header
	const UPCGRemoveInternalSettings* Settings = Context->GetInputSettings<UPCGRemoveInternalSettings>();
	check(Settings);
