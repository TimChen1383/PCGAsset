...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGEmitPoints)
//Using Name Space to avoid variable name conflict with engine code. Just add it
#define LOCTEXT_NAMESPACE "PCGEmitPoints"
...
FPCGElementPtr UPCGEmitPointsSettings::CreateElement() const
{
	return MakeShared<FPCGEmitPointsElement>();
}

bool FPCGEmitPointsElement::ExecuteInternal(FPCGContext* Context) const
{
	//Store PCG Settings. Use it for get access to all variables we declared or adjusted in PCG Settings in header
	const UPCGEmitPointsSettings* Settings = Context->GetInputSettings<UPCGEmitPointsSettings>();
	check(Settings);
	...
	const int32& EmitSeconds = Settings->EmitSeconds;
	const float& EmitVelocity = Settings->EmitVelocity;
	...
	for (const FPCGTaggedData& InputsTaggedData : InputsTaggedDatas)
	{
		...
		//How many seconds/points we want to spawn
		for(int EmitSecond = 0; EmitSecond< EmitSeconds; EmitSecond++)
		{
		...
		//Pass the function as parameter. This is a 2 inputs function: Index and PCG Point. Definition below
		{
			...
			FVector EmitVelocityDistance = SourceRotation.RotateVector(EmitDirection*EmitSecond);

			//Add original location
			FVector FinalPosition = SourceLocation + EmitVelocityDistance;

			//Add gravity distance
			float GravityDistance = Gravity*(EmitSecond*EmitSecond);
