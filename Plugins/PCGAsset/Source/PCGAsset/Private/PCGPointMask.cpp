...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGPointMask)
//Using Name Space to avoid variable name conflict with engine code. Just add it
#define LOCTEXT_NAMESPACE "PCGPointMask"
...
FPCGElementPtr UPCGPointMaskSettings::CreateElement() const
{
	return MakeShared<FPCGPointMaskElement>();

}

bool FPCGPointMaskElement::ExecuteInternal(FPCGContext* Context) const
{
		//Store PCG Settings. Use it for get access to all variables we declared or adjusted in PCG Settings in header
	const UPCGPointMaskSettings* Settings = Context->GetInputSettings<UPCGPointMaskSettings>();
	check(Settings);
