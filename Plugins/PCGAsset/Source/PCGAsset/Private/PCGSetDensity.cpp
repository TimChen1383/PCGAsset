...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGSetDensity)
//Using Name Space to avoid variable name conflict with engine code. Just add it
#define LOCTEXT_NAMESPACE "PCGSetDensity"
...
FPCGElementPtr UPCGSetDensitySettings::CreateElement() const
{
	return MakeShared<FPCGSetDensityElement>();
}

bool FPCGSetDensityElement::ExecuteInternal(FPCGContext* Context) const
{
	//Store PCG Settings. Use it for get access to all variables we declared or adjusted in PCG Settings in header
	const UPCGSetDensitySettings* Settings = Context->GetInputSettings<UPCGSetDensitySettings>();
	check(Settings);
	...
	const float& DensityValue = Settings->DensityValue;
