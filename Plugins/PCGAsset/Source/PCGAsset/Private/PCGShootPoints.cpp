...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGShootPoints)
//Using Name Space to avoid variable name conflict with engine code. Just add it
#define LOCTEXT_NAMESPACE "PCGShootPoints"
...
FPCGElementPtr UPCGShootPointsSettings::CreateElement() const
{
	return MakeShared<FPCGShootPointsElement>();
}

bool FPCGShootPointsElement::ExecuteInternal(FPCGContext* Context) const
{
	//Store PCG Settings. Use it for get access to all variables we declared or adjusted in PCG Settings in header
	const UPCGShootPointsSettings* Settings = Context->GetInputSettings<UPCGShootPointsSettings>();
	check(Settings);
	...
	for (const FPCGTaggedData& InputsTaggedData : InputsTaggedDatas)
	{
		...
		for(int ShootSecond = 0; ShootSecond< ShootSeconds; ShootSecond++)
		{
