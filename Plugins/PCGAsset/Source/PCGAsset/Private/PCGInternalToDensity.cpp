...
#include "PCGInternalToDensity.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(PCGInternalToDensity)
//Using Name Space to avoid variable name conflict with engine code. Just add it
#define LOCTEXT_NAMESPACE "PCGInternalToDensity"
...
FPCGElementPtr UPCGInternalToDensitySettings::CreateElement() const
{
	return MakeShared<FPCGInternalToDensityElement>();
}

bool FPCGInternalToDensityElement::ExecuteInternal(FPCGContext* Context) const
{
	//Store PCG Settings. Use it for get access to all variables we declared or adjusted in PCG Settings in header
	const UPCGInternalToDensitySettings* Settings = Context->GetInputSettings<UPCGInternalToDensitySettings>();
	check(Settings);
	...
	const float& SearchDistance = Settings->SearchDistance;
	const int32& RemoveThreshold = Settings->RemoveThreshold;
	...
	for (const FPCGTaggedData& InputsTaggedData : InputsTaggedDatas)
	{
		...
		//Pass the function as parameter. This is a 2 inputs function: Index and PCG Point. Definition below
		{
			...
			********************************************/
			FVector SourceLoc = InputPoint.Transform.GetLocation();
			int32 NeighborCounter = 0;

			for(int32 NeighborPoint = 0; NeighborPoint < InputPoints.Num(); NeighborPoint++)
			{
				FVector SearchPointLoc = InputPoints[NeighborPoint].Transform.GetLocation();
				if(FVector::Distance(SourceLoc,SearchPointLoc) < SearchDistance)
				{
					NeighborCounter++;
				}
			}

			if(NeighborCounter >= RemoveThreshold)
			{
				OutPoint.Density = 0;
			}
			else
